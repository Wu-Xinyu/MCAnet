...
def evaluate(conf_mat):
    ...
    acc_per_class = np.diag(conf_mat) / conf_mat.sum(axis=1)  #list
    acc_cls = np.nanmean(acc_per_class)  #每一类的平均准确率

    IoU = np.diag(conf_mat) / (conf_mat.sum(axis=1) + conf_mat.sum(axis=0) - np.diag(conf_mat))  #list
