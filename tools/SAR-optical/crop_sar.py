...
class image_to_patch:
    ...
    def to_patch(self):
        ...
        sar_files = sorted(os.listdir(self.sar_path))  #对文件夹中的文件进行排序，返回一个列表
        optical_files = sorted(os.listdir(self.optical_path))

        print(len(sar_files), len(optical_files))  #输出文件个数
...
if __name__ == '__main__':
    ...
    sar_path = r'whu-opt-sar\\sar'
    crop_sar_image_path = r'whu-opt-sar\crop\sar'

    optical_path = r'whu-opt-sar\\optical'
    crop_optical_image_path = r'whu-opt-sar\\crop\\optical'
